...
namespace POS.Application.Services
{
    public class CategoryApplication : ICategoryApplication
    {
        ...
        private readonly CategoryValidator _validationsRules;

        public CategoryApplication(IUnitOfWork unitOfWork, IMapper mapper, CategoryValidator validationsRules)
        {
        ...
        public async Task<BaseResponse<BaseEntityResponse<CategoryResponseDto>>> ListCategory(BaseFiltersRequest filters)
        {
            ...
            var categories = await _unitOfWork.Category.ListCategory(filters);
        ...
        public async Task<BaseResponse<bool>> RegisterCategory(CategoryRequestDto requestDto)
        {
            ...
            var validationResult = await _validationsRules.ValidateAsync(requestDto);

            if (!validationResult.IsValid)
            {
                response.IsSuccess = false;
                response.Message = ReplyMessage.MESSAGE_ERR_VALIDATE;
        ...
        {
            var response = new BaseResponse<bool>();
            var categoriesEdit = await CategoryById(categoryId);

            if (categoriesEdit.Data is null)
            {
                response.IsSuccess = false;
                response.Message = ReplyMessage.MESSAGE_QUERY_EMPTY;
            }
        ...
        {
            var response = new BaseResponse<bool>();
            var categoriesRemove = await CategoryById(categoryId);

            if (categoriesRemove.Data is null)
            {
                response.IsSuccess = false;
                response.Message = ReplyMessage.MESSAGE_QUERY_EMPTY;
            }
